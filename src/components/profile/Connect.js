...
const Connect = ({
  ...
  <div>
    <Button onClick={() => store(email, proLoggedIn, userName, userId)}>
      <a href={`https://connect.stripe.com/express/oauth/authorize?redirect_uri=http://165.227.53.23:4000/${id}/${name}&client_id=ca_CE8A2d9pdtIwopHsbTEvkKap1Hrl1ifV`}>Take credit card payments with Stripe</a>
    </Button>
  </div>
)
