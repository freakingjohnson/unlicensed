...
const nonProLogin = ({
  ...
  <div> Enter your email and password
    <FormGroup>
      <TextField label="Email" name="email" value={email} onChange={e => setInfo(e)} />
      <TextField label="Password" name="password" type="password" value={password} onChange={e => setInfo(e)} />
    </FormGroup>
    <Button raised color="primary" disabled={!email || !password}>Login</Button>
    <Divider />
    <div>
      New User?
      <Button raised color="primary" component={Link} to="/signup">
        Sign-up
      </Button>
    </div>
  </div>)
